...
| `03-stage-planning-execution` | 将课题拆成 3–5 个阶段，并控制一次只完成一个可验收工作包 |
...
> 调用科研项目总控 Skill。我要完成【任务】。先不要直接生成最终成果，请先检查已有信息、主动检索能够自行查明的内容，再向我询问真正影响方案质量的关键信息；随后将任务拆成 3–5 个阶段，经我确认后逐步执行。每阶段完成后进行质量校核并让我确认，再进入下一阶段。

### 2. 默认运行模式

- **标准模式（默认）**：3–7 个高价值问题；3–5 个阶段；关键节点由用户确认。
- **快速模式**：只问阻断性问题；2–3 个阶段；适合低风险、短任务。
