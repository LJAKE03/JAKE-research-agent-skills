...
| 日期 | 版本 / 状态 | 主要改进 |
|---|---|---|
| 2026-09-14 | `v2.5.0` 开发中 | 新增项目级模型选择：总控可选 Astra 或 Sol，证据与输出 Worker 可选 Sol、Terra 或 Luna；保留 Sol/xhigh 默认值，加入模型目录预检，并减少短任务中的强制委派、阶段确认和重复质量门。 |
| 2026-08-13 | `v2.4.1` 开发中 | 将 Luna 路由扩展到结论已锁定的单一成品输出；继续由 Terra 处理证据核验、Sol 处理科研判断，并明确 PDF、Word、Excel、PPT 后缀本身不决定模型。 |
...
### 2026-09-14 · `v2.5.0` 开发中

- **模型可由项目选择**：总控支持 Astra / Sol，证据与输出 Worker 支持 Sol / Terra / Luna；默认仍为 Sol/xhigh、Terra/medium、Luna/low。
- **配置彼此隔离**：选择写入项目 `.research-agent/MODEL_ROUTING.selection.json`，不会覆盖公共默认值，并只对新启动任务生效。
- **启动前验证**：初始化器检查模型与 reasoning effort 是否存在；当前运行时缺少 Astra 时保存配置并明确阻止启动。
- **减少额外工作**：简单任务直接完成，只有真实依赖或收益高于调用与交接成本时才委派；质量门与用户确认集中在高影响节点。
- **完整技术记录**：[项目级模型选择与轻量执行](update-history/2026-09-14-configurable-model-routing.md)。

### 2026-08-13 · `v2.4.1` 开发中

- **锁定成品优先交给 Luna**：内容、字段、顺序、模板和决策均已确定时，可由 Luna 组织单一低判断成品。
- **职责边界保持清晰**：外部检索与事实提取仍由 Terra 负责，计算、方法选择、分类和关键解释仍由 Sol 负责。
- **文件类型不决定路由**：PDF、Word、Excel 或 PPT 只是交付格式，是否仍需科研判断才是模型选择依据。
- **完整技术记录**：[Luna 单一低判断成品路由](update-history/2026-08-13-luna-single-output-routing.md)。
