...
- 跨文件变更遵循 `shared/CHANGE_INTEGRITY_PROTOCOL.md`，先确定 canonical、generated、consumer、historical 和 temporary 边界。

## 修改与发布

- 修改前必须建立或更新回归测试；测试至少覆盖触发、路由、输出边界或安全行为中的相关项。
- 优先修复根因并收敛到唯一权威来源；generated 文件通过生成链更新，兼容层必须有验证方式和退出条件。
- 经用户确认后，才可合并为正式版本。
