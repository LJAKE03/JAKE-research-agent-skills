...
- 新增主动调研、关键提问、选择性记忆授权、重复习惯优化和当前指令优先级评测；与投稿追溯评测合并后总数为 28。
...
- 项目复盘和个人记忆库改用分级记录：A/B 级重要原则与工作流保留原因、步骤、边界和例外；C/D 级只保留关键步骤或留在项目内。
- 重复出现只提高经验候选优先级，不再使用机械次数阈值替代用户确认；当前明确指令始终优先于历史记忆。
- 项目收尾改为 Sol 先主动生成候选摘要再询问授权；用户未回复不得写入全局记忆，已完成交付物不因记忆待确认而被扣留。
