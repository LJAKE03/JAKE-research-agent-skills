...
foreach($name in $names){
  ...
  if(-not(Test-Path -LiteralPath $dst)){New-Item -ItemType Directory -Path $dst -Force|Out-Null; Copy-Item -LiteralPath (Join-Path $src '*') -Destination $dst -Recurse; Write-Output "NEW $name"; continue}
  $item=Get-Item -LiteralPath $dst -Force
  if($item.LinkType){Write-Output "SKIP $name (链接安装，无需同步)"; continue}
  $before=@(Get-ChildItem -LiteralPath $dst -Recurse -File).Count
  if($PSCmdlet.ShouldProcess($dst,"复制更新 $name")){Copy-Item -LiteralPath (Join-Path $src '*') -Destination $dst -Recurse -Force}
  $after=@(Get-ChildItem -LiteralPath $dst -Recurse -File).Count
  Write-Output "UPDATED $name (files before=$before after=$after)"
  if($Prune){
    Write-Warning "已启用 -Prune：将删除目标中不在正式源的额外文件。"
    $srcRel=@(Get-ChildItem -LiteralPath $src -Recurse -File | ForEach-Object {$_.FullName.Substring($src.Length).TrimStart('\')})
    Get-ChildItem -LiteralPath $dst -Recurse -File | ForEach-Object { $rel=$_.FullName.Substring($dst.Length).TrimStart('\'); if($srcRel -notcontains $rel -and $PSCmdlet.ShouldProcess($_.FullName,'删除额外文件')){Remove-Item -LiteralPath $_.FullName -Force} }
  }
}
